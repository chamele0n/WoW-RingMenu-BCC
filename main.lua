...
function RingMenu_UpdateRing(ringID)
    ...
    if not RingMenu.ringFrame[ringID] then
        ...
        rf.backdrop:SetTexture(RingMenu.addonPath .. "RingMenuBackdrop.tga")
    ...
    for buttonID = 1, (config.numSlots or 1) do
        ...
        button:SetAttribute("closeOnClick", config.closeOnClick)
        RingMenu.SetActionButtonSlot(button, buttonSlot)
        button:Show()
    end
    -- Hide unused buttons
